...
then
	...
	log_folder=${LOG_PATH}log_${prefix}_${jname}_${db}
...
#			fi
            ...
            if [[ -n ${SLURM_ADD_OPT} ]] ## must be space separated arguments
            then
            	for x in ${SLURM_ADD_OPT}
            	do
            		if [[ -n ${x} ]] 
            		then 
            			echo "#SBATCH $x" >>  ${file}.slurm
            		fi
            	done
            fi
...
then 
	...
	then
		echo "sbatch${appAccount} ${SLURM_ADD_OPT} --job-name=${PROJECT_ID}_p${currentPhase}s${currentStep+1} -o ${log_folder}${prefix}_step${currentStep}_${RAW_DB%.db}_${slurmID}.${resumeIdx}.out -e ${log_folder}${prefix}_step${currentStep}_${RAW_DB%.db}_${slurmID}.${resumeIdx}.err -n1 -c1 -p ${SLURM_PARTITION} --time=01:00:00 --mem-per-cpu=1g --dependency=afterok:${RET##* } --wrap=\"${ADDCMD}bash ${SUBMIT_SCRIPTS_PATH}/createAndSubmitMarvelSlurmJobs.sh ${configFile} ${currentPhase} ${currentStep} $slurmID $((${resumeIdx}+1))\"" 
		sbatch${appAccount} ${SLURM_ADD_OPT} --job-name=${PROJECT_ID}_p${currentPhase}s${currentStep+1} -o ${log_folder}${prefix}_step${currentStep}_${RAW_DB%.db}_${slurmID}.${resumeIdx}.out -e ${log_folder}${prefix}_step${currentStep}_${RAW_DB%.db}_${slurmID}.${resumeIdx}.err -n1 -c1 -p ${SLURM_PARTITION} --time=01:00:00 --mem-per-cpu=1g --dependency=afterok:${RET##* } --wrap="${ADDCMD}bash ${SUBMIT_SCRIPTS_PATH}/createAndSubmitMarvelSlurmJobs.sh ${configFile} ${currentPhase} ${currentStep} $slurmID $((${resumeIdx}+1))"
		foundNext=1
	fi	
fi
...
then
    ...
    then
    	echo "sbatch${appAccount} ${SLURM_ADD_OPT} --job-name=${PROJECT_ID}_p${currentPhase}s$((${currentStep+1})) -o ${prefix}_step$((${currentStep}+1))_${db}_${slurmID}.out -e ${log_folder}${prefix}_step$((${currentStep}+1))_${db%.db}_${slurmID}.err -n1 -c1 -p ${SLURM_PARTITION} --time=01:00:00 --mem-per-cpu=1g --dependency=afterok:${RET##* } --wrap=\"${ADDCMD}bash ${SUBMIT_SCRIPTS_PATH}/createAndSubmitMarvelSlurmJobs.sh ${configFile} ${currentPhase} $((${currentStep}+1)) $slurmID\""
    	sbatch${appAccount} ${SLURM_ADD_OPT} --job-name=${PROJECT_ID}_p${currentPhase}s$((${currentStep+1})) -o ${log_folder}${prefix}_step$((${currentStep}+1))_${db}_${slurmID}.out -e ${log_folder}${prefix}_step$((${currentStep}+1))_${db%.db}_${slurmID}.err -n1 -c1 -p ${SLURM_PARTITION} --time=01:00:00 --mem-per-cpu=1g --dependency=afterok:${RET##* } --wrap="${ADDCMD}bash ${SUBMIT_SCRIPTS_PATH}/createAndSubmitMarvelSlurmJobs.sh ${configFile} ${currentPhase} $((${currentStep}+1)) $slurmID"
        foundNext=1
	else
...
then
    ...
    then
    	echo "sbatch${appAccount} ${SLURM_ADD_OPT} --job-name=${PROJECT_ID}_p${currentPhase}s$((${currentStep+1})) -o ${log_folder}${prefix}_step$((${currentStep}+1))_${db}_${slurmID}.out -e ${log_folder}${prefix}_step$((${currentStep}+1))_${db%.db}_${slurmID}.err -n1 -c1 -p ${SLURM_PARTITION} --time=01:00:00 --mem-per-cpu=1g --dependency=afterok:${RET##* } --wrap=\"${ADDCMD}bash ${SUBMIT_SCRIPTS_PATH}/createAndSubmitMarvelSlurmJobs.sh ${configFile} ${currentPhase} $((${currentStep}+1)) $slurmID\""
    	sbatch${appAccount} ${SLURM_ADD_OPT} --job-name=${PROJECT_ID}_p${currentPhase}s$((${currentStep+1})) -o ${log_folder}${prefix}_step$((${currentStep}+1))_${db}_${slurmID}.out -e ${log_folder}${prefix}_step$((${currentStep}+1))_${db%.db}_${slurmID}.err -n1 -c1 -p ${SLURM_PARTITION} --time=01:00:00 --mem-per-cpu=1g --dependency=afterok:${RET##* } --wrap="${ADDCMD}bash ${SUBMIT_SCRIPTS_PATH}/createAndSubmitMarvelSlurmJobs.sh ${configFile} ${currentPhase} $((${currentStep}+1)) $slurmID"
        foundNext=1
	else
		### we have to create the coverage directory and change into that dir 
...
then 
    ...
    then
    	echo "sbatch${appAccount} ${SLURM_ADD_OPT} --job-name=${PROJECT_ID}_p${currentPhase}s$((${currentStep+1})) -o ${log_folder}${prefix}_step$((${currentStep}+1))_${RAW_DB%.db}_${slurmID}.out -e ${log_folder}${prefix}_step$((${currentStep}+1))_${RAW_DB%.db}_${slurmID}.err -n1 -c1 -p ${SLURM_PARTITION} --time=01:00:00 --mem-per-cpu=1g --dependency=afterok:${RET##* } --wrap=\"${ADDCMD}bash ${SUBMIT_SCRIPTS_PATH}/createAndSubmitMarvelSlurmJobs.sh ${configFile} ${currentPhase} $((${currentStep}+1)) $slurmID\"" 
        sbatch${appAccount} ${SLURM_ADD_OPT} --job-name=${PROJECT_ID}_p${currentPhase}s$((${currentStep+1})) -o ${log_folder}${prefix}_step$((${currentStep}+1))_${RAW_DB%.db}_${slurmID}.out -e ${log_folder}${prefix}_step$((${currentStep}+1))_${RAW_DB%.db}_${slurmID}.err -n1 -c1 -p ${SLURM_PARTITION} --time=01:00:00 --mem-per-cpu=1g --dependency=afterok:${RET##* } --wrap="${ADDCMD}bash ${SUBMIT_SCRIPTS_PATH}/createAndSubmitMarvelSlurmJobs.sh ${configFile} ${currentPhase} $((${currentStep}+1)) $slurmID"
        foundNext=1
    else 
        currentPhase=1
        currentStep=$((${RAW_REPMASK_SUBMIT_SCRIPTS_FROM}-1))
...
then 
    ...
    then
    	echo "sbatch${appAccount} ${SLURM_ADD_OPT} --job-name=${PROJECT_ID}_p${currentPhase}s$((${currentStep+1})) -o ${log_folder}${prefix}_step$((${currentStep}+1))_${FIX_DB%.db}_${slurmID}.out -e ${log_folder}${prefix}_step$((${currentStep}+1))_${FIX_DB%.db}_${slurmID}.err -n1 -c1 -p ${SLURM_PARTITION} --time=01:00:00 --mem-per-cpu=1g --dependency=afterok:${RET##* } --wrap=\"${ADDCMD}bash ${SUBMIT_SCRIPTS_PATH}/createAndSubmitMarvelSlurmJobs.sh ${configFile} ${currentPhase} $((${currentStep}+1)) $slurmID\"" 
        sbatch${appAccount} ${SLURM_ADD_OPT} --job-name=${PROJECT_ID}_p${currentPhase}s$((${currentStep+1})) -o ${log_folder}${prefix}_step$((${currentStep}+1))_${FIX_DB%.db}_${slurmID}.out -e ${log_folder}${prefix}_step$((${currentStep}+1))_${FIX_DB%.db}_${slurmID}.err -n1 -c1 -p ${SLURM_PARTITION} --time=01:00:00 --mem-per-cpu=1g --dependency=afterok:${RET##* } --wrap="${ADDCMD}bash ${SUBMIT_SCRIPTS_PATH}/createAndSubmitMarvelSlurmJobs.sh ${configFile} ${currentPhase} $((${currentStep}+1)) $slurmID"
        foundNext=1
    else 
        currentPhase=5
        currentStep=$((${FIX_FILT_SUBMIT_SCRIPTS_FROM}-1))
...
then 
    ...
    then
    	echo "sbatch${appAccount} ${SLURM_ADD_OPT} --job-name=${PROJECT_ID}_p${currentPhase}s${currentStep} -o ${log_folder}${prefix}_step$((${currentStep}+1))_${FIX_DB%.db}_${slurmID}.out -e ${log_folder}${prefix}_step$((${currentStep}+1))_${FIX_DB%.db}_${slurmID}.err -n1 -c1 -p ${SLURM_PARTITION} --time=01:00:00 --mem-per-cpu=1g --dependency=afterok:${RET##* } --wrap=\"${ADDCMD}bash ${SUBMIT_SCRIPTS_PATH}/createAndSubmitMarvelSlurmJobs.sh ${configFile} ${currentPhase} ${currentStep} $slurmID\""                 
        sbatch${appAccount} ${SLURM_ADD_OPT} --job-name=${PROJECT_ID}_p${currentPhase}s${currentStep} -o ${log_folder}${prefix}_step$((${currentStep}+1))_${FIX_DB%.db}_${slurmID}.out -e ${log_folder}${prefix}_step$((${currentStep}+1))_${FIX_DB%.db}_${slurmID}.err -n1 -c1 -p ${SLURM_PARTITION} --time=01:00:00 --mem-per-cpu=1g --dependency=afterok:${RET##* } --wrap="${ADDCMD}bash ${SUBMIT_SCRIPTS_PATH}/createAndSubmitMarvelSlurmJobs.sh ${configFile} ${currentPhase} ${currentStep} $slurmID"
        foundNext=1
...
then 
    ...
    then
    	echo "sbatch${appAccount} ${SLURM_ADD_OPT} --job-name=${PROJECT_ID}_p${currentPhase}s$((${currentStep+1})) -o ${log_folder}${prefix}_step$((${currentStep}+1))_${CONT_DB%.db}_${slurmID}.out -e ${log_folder}${prefix}_step$((${currentStep}+1))_${CONT_DB%.db}_${slurmID}.err -n1 -c1 -p ${SLURM_PARTITION} --time=01:00:00 --mem-per-cpu=1g --dependency=afterok:${RET##* } --wrap=\"${ADDCMD}bash ${SUBMIT_SCRIPTS_PATH}/createAndSubmitMarvelSlurmJobs.sh ${configFile} ${currentPhase} $((${currentStep}+1)) $slurmID\"" 
        sbatch${appAccount} ${SLURM_ADD_OPT} --job-name=${PROJECT_ID}_p${currentPhase}s$((${currentStep+1})) -o ${log_folder}${prefix}_step$((${currentStep}+1))_${CONT_DB%.db}_${slurmID}.out -e ${log_folder}${prefix}_step$((${currentStep}+1))_${CONT_DB%.db}_${slurmID}.err -n1 -c1 -p ${SLURM_PARTITION} --time=01:00:00 --mem-per-cpu=1g --dependency=afterok:${RET##* } --wrap="${ADDCMD}bash ${SUBMIT_SCRIPTS_PATH}/createAndSubmitMarvelSlurmJobs.sh ${configFile} ${currentPhase} $((${currentStep}+1)) $slurmID"
        foundNext=1
	else 
        currentPhase=9
        currentStep=$((${PB_ARROW_SUBMIT_SCRIPTS_FROM}-1))        
...
then
	...
	echo "sbatch${appAccount} ${SLURM_ADD_OPT} --job-name=${PROJECT_ID}_final -o ${log_folder}${prefix}_final_step_${currentPhase}_${currentStep}_${slurmID}.out -e ${log_folder}${prefix}_final_step_${currentPhase}_${currentStep}_${slurmID}.err -n1 -c1 -p ${SLURM_PARTITION} --time=00:15:00 --mem=1g --dependency=afterok:${RET##* } --wrap=\"sleep 5 && echo \"finished - all selected jobs created and submitted. Last Step: ${prefix} ${currentPhase} ${currentStep} $slurmID ${configFile}\"\""
	sbatch${appAccount} ${SLURM_ADD_OPT} --job-name=${PROJECT_ID}_final -o ${log_folder}${prefix}_final_step_${currentPhase}_${currentStep}_${slurmID}.out -e ${log_folder}${prefix}_final_step_${currentPhase}_${currentStep}_${slurmID}.err -n1 -c1 -p ${SLURM_PARTITION} --time=00:15:00 --mem=1g --dependency=afterok:${RET##* } --wrap="sleep 5 && echo \"finished - all selected jobs created and submitted. Last Step: ${prefix} ${currentPhase} ${currentStep} $slurmID ${configFile}\""     
fi
