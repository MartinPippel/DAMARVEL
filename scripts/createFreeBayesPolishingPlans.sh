...
then 
    ...
    then
		...
		echo "samtools $(${CONDA_FREEBAYES_ENV} && samtools 2>&1 | grep Version | awk '{print $2}' && conda deactivate)" > freebayes_01_FBprepareInput_single_${CONT_DB}.${slurmID}.version
		echo "bwa $(${CONDA_FREEBAYES_ENV} && bwa 2>&1 | grep Version | awk '{print $2}' && conda deactivate)" >> freebayes_01_FBprepareInput_single_${CONT_DB}.${slurmID}.version
	### 2-FBfastp 
    ...
    then
		...
		then
			...
			do
	   		...
	   		echo "fastp $(${CONDA_FREEBAYES_ENV} && fastp 2>&1 | grep version | awk '{print $2}' && conda deactivate)" > freebayes_02_FBfastp_block_${CONT_DB}.${slurmID}.version	   		
		else		
    ...
    then
		...
		then
			...
			do
	   		...
	   		echo "bwa $(${CONDA_FREEBAYES_ENV} && bwa 2>&1 | grep Version | awk '{print $2}' && conda deactivate)" > freebayes_03_FBbwa_block_${CONT_DB}.${slurmID}.version
        	echo "samtools $(${CONDA_FREEBAYES_ENV} && samtools 2>&1 | grep Version | awk '{print $2}' && conda deactivate)" >> freebayes_03_FBbwa_block_${CONT_DB}.${slurmID}.version
		else		
			(>&2 echo "ERROR - set CT_FREEBAYES_READSTYPE ${CT_FREEBAYES_READSTYPE} not supported yet")
	        exit 1			
		fi        	
    ...
    then
        ...
        then
   		...
   		echo "picard MarkDuplicates $(${CONDA_FREEBAYES_ENV} && picard MarkDuplicates --version && conda deactivate)" > freebayes_04_FBmarkDuplicates_block_${CONT_DB}.${slurmID}.version
   		...
   		then
   			...
   			echo "picard MergeSamFiles $(${CONDA_FREEBAYES_ENV} && picard MarkDuplicates --version && conda deactivate)" >> freebayes_04_FBmarkDuplicates_block_${CONT_DB}.${slurmID}.version	   			
		else
...
then 
    ...
    then
		...
		echo "tr ':/' '_'  < ${CT_FREEBAYES_REFFASTA} | awk '{print \$1}' > ${CT_FREEBAYES_OUTDIR}/freebayes_${CT_FREEBAYES_RUNID}/ref/$(basename ${CT_FREEBAYES_REFFASTA})" >> freebayes_01_FBprepareInput_single_${CONT_DB}.${slurmID}.plan		
