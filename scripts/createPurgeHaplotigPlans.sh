...
then 
    ...
    then
		...
		echo "${PURGEDUPS_PATH}/bin/get_seqs -e -p ${CT_PURGEHAPLOTIGS_OUTDIR}/purgeHaplotigs_${CT_PURGEHAPLOTIGS_RUNID}/${ref%.fasta} ${CT_PURGEHAPLOTIGS_OUTDIR}/purgeHaplotigs_${CT_PURGEHAPLOTIGS_RUNID}/dups.bed ${CT_PURGEHAPLOTIGS_INFASTA}" >> purgeHaplotigs_05_purgedups_single_${CONT_DB}.${slurmID}.plan 
...
then 
    ...
    then
		...
		echo "grep -e \">\" ${CT_PURGEHAPLOTIGS_OUTDIR}/purgeHaplotigs_${CT_PURGEHAPLOTIGS_RUNID}/${ref}.fasta | awk '{print \$1}' | tr -d \">\" > ${CT_PURGEHAPLOTIGS_OUTDIR}/purgeHaplotigs_${CT_PURGEHAPLOTIGS_RUNID}/${ref}.header" >> purgeHaplotigs_01_TCPrepInput_single_${CONT_DB}.${slurmID}.plan
    ...
    then
        ...
        do 
           	echo "cd ${CT_PURGEHAPLOTIGS_OUTDIR}/purgeHaplotigs_${CT_PURGEHAPLOTIGS_RUNID} && ${MARVEL_PATH}/bin/DBdust ${PROJECT_ID}_CT_M.${x} && ${DAZZLER_PATH}/bin/DBdust ${PROJECT_ID}_CT_Z.${x} && ${DAZZLER_PATH}/bin/DBdust ${PROJECT_ID}_CT_Z.${x} && cd ${myCWD}"
    ...
    then
		...
		if [[ -n ${CT_PURGEHAPLOTIGS_DATANDER_VERBOSE} && ${CT_PURGEHAPLOTIGS_DATANDER_VERBOSE} -ne "0" ]]
    	then
        	CT_PURGEHAPLOTIGS_DATANDER_OPT="${CT_PURGEHAPLOTIGS_DATANDER_OPT} -v"
    	fi
		if [[ -n ${CT_PURGEHAPLOTIGS_DATANDER_KMER} ]]
    	then
        	CT_PURGEHAPLOTIGS_DATANDER_OPT="${CT_PURGEHAPLOTIGS_DATANDER_OPT} -k${CT_PURGEHAPLOTIGS_DATANDER_KMER}"
    	fi
		if [[ -n ${CT_PURGEHAPLOTIGS_DATANDER_WIDTH} ]]
    	then
        	CT_PURGEHAPLOTIGS_DATANDER_OPT="${CT_PURGEHAPLOTIGS_DATANDER_OPT} -w${CT_PURGEHAPLOTIGS_DATANDER_WIDTH}"
    	fi
    ...
    then
		...
		if [[ -n "${CT_PURGEHAPLOTIGS_DALIGNER_MASK}" ]]
		then 
			for x in ${CT_PURGEHAPLOTIGS_DALIGNER_MASK}
			do
				CT_PURGEHAPLOTIGS_DALIGNER_OPT="${CT_PURGEHAPLOTIGS_DALIGNER_OPT} -m${CT_PURGEHAPLOTIGS_DALIGNER_MASK}"
			done
		fi
    ...
    then
    	...
    	if [[ -n ${CT_PURGEHAPLOTIGS_LAFILTERCHAINS_CBEG} ]]
    	then
        	CT_PURGEHAPLOTIGS_LAFILTERCHAINS_OPT="${CT_PURGEHAPLOTIGS_LAFILTERCHAINS_OPT} -u${CT_PURGEHAPLOTIGS_LAFILTERCHAINS_CBEG}"
    	fi
