...
function setDalignerOptions()
{
    ...
    if [[ -n ${COR_CONTIG_DALIGNER_BRIDGE} && ${COR_CONTIG_DALIGNER_BRIDGE} -eq 1 ]]
    then
    ...
    if [[ -n ${COR_CONTIG_DALIGNER_OLEN} && ${COR_CONTIG_DALIGNER_OLEN} -ne 0 ]]
    then
    ...
    if [[ -n ${COR_CONTIG_DALIGNER_TMP} ]]
    then
        CONTIG_DALIGNER_OPT="${CONTIG_DALIGNER_OPT} -P${COR_CONTIG_DALIGNER_TMP}"
    fi        
...
then 
    ...
    then
        ...
        for x in $(seq 1 ${contigblocks}); 
        do  
            echo "${MARVEL_PATH}/bin/LAmerge${CONTIG_LAMERGE_OPT} ${FIX_FILT_OUTDIR}/${ANALYZE_DIR}/${CONT_DB%.db} ${FIX_FILT_OUTDIR}/${ANALYZE_DIR}/${CONT_DB%.db}.${x}.las ${FIX_FILT_OUTDIR}/${ANALYZE_DIR}/d${x}"
