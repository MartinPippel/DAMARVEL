...
typedef enum
{
	sort_abpos = 0, sort_aepos = 1, sort_length = 2, sort_readID = 3, sort_qual = 4, sort_phase = 5
} sortType;

typedef enum
{
	show_q = 0, show_distinct = 1, show_same = 2, show_phase = 3
} showType;
...
typedef struct
{
	...
	// tracks of phasing
	HITS_TRACK *phase_SCtrack;
	HITS_TRACK *phase_PStrack;
	HITS_TRACK *phase_HPtrack;
...
static int cmp_ovls_phase(const void *a, const void *b)
{
	OverlapDetails *o1 = *(OverlapDetails**) a;
	OverlapDetails *o2 = *(OverlapDetails**) b;
	...
	if (g_ectx.phase_SCtrack && g_ectx.phase_HPtrack && g_ectx.phase_PStrack)
	{
		int o1_phaseSCb, o1_phaseHPb, o1_phasePSb;
		o1_phaseSCb = o1_phaseHPb = o1_phasePSb = -1;
		int o2_phaseSCb, o2_phaseHPb, o2_phasePSb;
		o2_phaseSCb = o2_phaseHPb = o2_phasePSb = -1;

		track_anno *phaseanno = g_ectx.phase_SCtrack->anno;
		track_data *phasedata = g_ectx.phase_SCtrack->data;

		if (phaseanno[o1->ovl.bread] >= phaseanno[o1->ovl.bread + 1])
			o1_phaseSCb = -1;
		else
			o1_phaseSCb = phasedata[phaseanno[o1->ovl.bread] / sizeof(track_data)];

		if (phaseanno[o2->ovl.bread] >= phaseanno[o2->ovl.bread + 1])
			o2_phaseSCb = -1;
		else
			o2_phaseSCb = phasedata[phaseanno[o2->ovl.bread] / sizeof(track_data)];

		phaseanno = g_ectx.phase_HPtrack->anno;
		phasedata = g_ectx.phase_HPtrack->data;

		if (phaseanno[o1->ovl.bread] >= phaseanno[o1->ovl.bread + 1])
			o1_phaseHPb = -1;
		else
			o1_phaseHPb = phasedata[phaseanno[o1->ovl.bread] / sizeof(track_data)];

		if (phaseanno[o2->ovl.bread] >= phaseanno[o2->ovl.bread + 1])
			o2_phaseHPb = -1;
		else
			o2_phaseHPb = phasedata[phaseanno[o2->ovl.bread] / sizeof(track_data)];

		phaseanno = g_ectx.phase_PStrack->anno;
		phasedata = g_ectx.phase_PStrack->data;

		if (phaseanno[o1->ovl.bread] >= phaseanno[o1->ovl.bread + 1])
			o1_phasePSb = -1;
		else
			o1_phasePSb = phasedata[phaseanno[o1->ovl.bread] / sizeof(track_data)];

		if (phaseanno[o2->ovl.bread] >= phaseanno[o2->ovl.bread + 1])
			o2_phasePSb = -1;
		else
			o2_phasePSb = phasedata[phaseanno[o2->ovl.bread] / sizeof(track_data)];

		cmp = o1_phaseSCb - o2_phaseSCb;

		if (cmp == 0)
		{
			cmp = o1_phaseHPb - o2_phaseHPb;
		}

		if (cmp == 0)
		{
			cmp = o1_phasePSb - o2_phasePSb;
		}

	}
	else // fallback to abeg
	{
...
static void adjust_drawing_area_size()
{
	...
	gtk_widget_set_size_request(g_ectx.drawing_area, pwidth, (g_ectx.novl_display + 1) * g_ectx.line_width * (g_ectx.pad + 1) + PADDING);
}
...
static int xpos2apos(int x)
{
	...
	int ovlALen;
	if (g_ectx.ocur > 0)
		ovlALen = DB_READ_LEN(&g_ectx.db, g_ectx.ovls->ovl.aread);
	else
		ovlALen = DB_READ_LEN(&g_ectx.db, g_ectx.db.nreads - 1);
...
static void sort_overlaps()
{
	...
	switch (g_ectx.sort)
	{
		...
		case sort_phase:
			qsort(ovls_sorted, n, sizeof(OverlapDetails*), cmp_ovls_phase);
			break;
...
static void draw_ovl(cairo_t *cr, OverlapDetails *ovld, int y, float scale, int clip_xb, int clip_xe, int bHighlight)
{
	...
	else if (g_ectx.show == show_phase)
	{
		if (g_ectx.phase_SCtrack && g_ectx.phase_HPtrack && g_ectx.phase_PStrack)
		{
			...
			if (phaseanno[ovl->bread] >= phaseanno[ovl->bread + 1])
				phaseSCb = 0;
			else
				phaseSCb = phasedata[phaseanno[ovl->bread] / sizeof(track_data)];
			...
			if (phaseanno[ovl->bread] >= phaseanno[ovl->bread + 1])
				...
				phaseHPb = phasedata[phaseanno[ovl->bread] / sizeof(track_data)];
			...
			if (phaseanno[ovl->bread] >= phaseanno[ovl->bread + 1])
				...
				phasePSb = phasedata[phaseanno[ovl->bread] / sizeof(track_data)];

			float base = (1.0 / (1 + phaseSCb) + 1.0 / (1 + phaseHPb) + 1.0 / (1 + phasePSb)) / 3.0;

			if (bHighlight)
			{
				base = 1 - (1.0 / (1 + phaseSCb) + 1.0 / (1 + phaseHPb) + 1.0 / (1 + phasePSb)) / 3.0;
			}

			if (bHighlight)
				cairo_set_source_rgb(cr, 1.0 - fabs(1.0 / phaseSCb), 1.0 - fabs(1.0 / phaseHPb), 1.0 - fabs(1.0 / phasePSb));
			else
				cairo_set_source_rgb(cr, fabs(1.0 / phaseSCb), fabs(1.0 / phaseHPb), fabs(1.0 / phasePSb));
		}
		else // fallback to same
		{
			int base = 0;
...
static gboolean read_draw_callback(GtkWidget *widget, cairo_t *cr, gpointer data)
{
	...
	for (i = draw_start; i < draw_end; i++)
	{
		...
		if (ovl->bread == g_ectx.rid_hover_highlight || (g_ectx.filter == filter_highlight && ovl->flags & g_ectx.filter_mask))
		{
			highlight = 1;
		}
...
static gboolean on_key_press(GtkWidget *widget, GdkEventKey *event, gpointer user_data)
{
	...
	switch (event->keyval)
	{
		...
		case GDK_KEY_p:
			g_ectx.show = show_phase;
			gtk_combo_box_set_active_id(GTK_COMBO_BOX(g_ectx.combo_show), "phase");
			redraw();
			break;
...
static void combo_show_changed(GtkComboBox *widget, gpointer user_data)
{
	...
	else if (strcmp(gtk_combo_box_get_active_id(widget), "phase") == 0)
	{
		g_ectx.show = show_phase;
	}
	else
...
static GtkWidget* setup_toolbar()
{
	...
	gtk_combo_box_text_append(GTK_COMBO_BOX_TEXT(combo_show), "phase", "Phase");
	...
	gtk_combo_box_text_append(GTK_COMBO_BOX_TEXT(combo_sort), "phase", "Phase");
...
static void update_details_bar(int x, int idx)
{
	...
	if (g_ectx.phase_SCtrack && g_ectx.phase_HPtrack && g_ectx.phase_PStrack)
	{
		int phaseSCa, phaseSCb, phaseHPa, phaseHPb, phasePSa, phasePSb;

		track_anno *phaseanno = g_ectx.phase_SCtrack->anno;
		track_data *phasedata = g_ectx.phase_SCtrack->data;

		if (phaseanno[ovl->aread] >= phaseanno[ovl->aread + 1])
			phaseSCa = -1;
		else
			phaseSCa = phasedata[phaseanno[ovl->aread] / sizeof(track_data)];
		...
		if (phaseanno[ovl->aread] >= phaseanno[ovl->aread + 1])
			...
			phaseHPa = phasedata[phaseanno[ovl->aread] / sizeof(track_data)];
		...
		if (phaseanno[ovl->aread] >= phaseanno[ovl->aread + 1])
			...
			phasePSa = phasedata[phaseanno[ovl->aread] / sizeof(track_data)];
		...
		sprintf(phase, " PH [%3d, %3d, %3d|%3d, %3d, %3d]", phaseSCa, phaseHPa, phasePSa, phaseSCb, phaseHPb, phasePSb);
	}
	else
	{
		phase[0] = '\0';
	...
	gchar *text = g_markup_printf_escaped("<tt>@ %5dbp cov %3d%s <b>%8d</b> (%5d) x <b>%8d</b> (%5d) <b>%5d..%5d</b> (%5d) %c <b>%5d..%5d</b> (%5d)%s %s flags %s</tt>", x, cov, q, ovl->aread, DB_READ_LEN(&g_ectx.db, ovl->aread), ovl->bread, DB_READ_LEN(&g_ectx.db, ovl->bread), ovl->path.abpos,
			ovl->path.aepos, ovl->path.aepos - ovl->path.abpos, ovl->flags & OVL_COMP ? '<' : '>', ovl->path.bbpos, ovl->path.bepos, ovl->path.bepos - ovl->path.bbpos, src, phase, flags);
...
static int command_line(GApplication *application, GApplicationCommandLine *cmdline)
{
	...
	{ "phase", 'p', 0, G_OPTION_ARG_INT, &phase, "load phase tracks (ScaffIdx, PS, HP)", NULL },
	...
	if (phase != 0)
	{
		if (!(g_ectx.phase_HPtrack = track_load(&g_ectx.db, "HP")))
		{
			printf("failed to load track HP\n");
			exit(1);
		}
		if (!(g_ectx.phase_PStrack = track_load(&g_ectx.db, "PS")))
		{
			printf("failed to load track PS\n");
			exit(1);
		}
		if (!(g_ectx.phase_SCtrack = track_load(&g_ectx.db, "ScaffIdx")))
		{
			printf("failed to load track ScaffIdx\n");
			exit(1);
		}
	}
